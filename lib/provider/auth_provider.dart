...
class AuthProvider with ChangeNotifier {
  ...
  Future registration(
      RegisterModel register, Function callback, BuildContext context) async {
    _isLoading = true;
    notifyListeners();
  ...
  Future login(LoginModel body, Function callback, BuildContext context) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await authRepo.login(body);
    _isLoading = false;
    if (apiResponse.response != null &&
        apiResponse.response.statusCode == 200) {
      Map map = apiResponse.response.data;
