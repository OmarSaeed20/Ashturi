  ...
  Future<void> getLatestProductList(int offset, BuildContext context,
    ...
    if (!_offsetList.contains(offset)) {
      ...
      if (apiResponse.response != null &&
        ...
        List<dynamic> products = apiResponse.response.data['products'];
        ...
        if (AppConstants.STATE.isNotEmpty) {
          for (var product in products) {
            for (var element in product['choice_options']) {
              for (var element in element['options']) {
                if (element == AppConstants.STATE) {
                  _lProductList.add(Product.fromJson(product));
                  _lPageSize = ProductModel.fromJson(apiResponse.response.data)
                      .totalSize;
                  _firstLoading = false;
                  _isLoading = false;
                }
      ...
      if (AppConstants.STATE.isNotEmpty) {
        for (var product in products) {
          for (var element in product['choice_options']) {
            for (var element in element['options']) {
              if (element == AppConstants.STATE) {
                _sellerProductList.add(Product.fromJson(product));
                _sellerAllProductList.add(Product.fromJson(product));
                _sellerPageSize =
                    ProductModel.fromJson(apiResponse.response.data).totalSize;
                _firstLoading = false;
                _filterIsLoading = false;
        ...
        if (AppConstants.STATE.isNotEmpty) {
          for (var product in products) {
            for (var element in product['choice_options']) {
              for (var element in element['options']) {
                if (element == AppConstants.STATE) {
                  _featuredProductList.add(Product.fromJson(product));
                  _featuredPageSize =
                      ProductModel.fromJson(apiResponse.response.data)
                          .totalSize;
                  _firstFeaturedLoading = false;
  ...
  Future<void> getRecommendedProduct(BuildContext context) async {
    ...
    if (apiResponse.response != null &&
      ...
      if (_recommendedProduct != null) {
        print('Recommended ----->${recommendedProduct.toJson()}');
      }
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
