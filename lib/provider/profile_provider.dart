...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<String> getUserInfo(BuildContext context) async {
    ...
    if (apiResponse.response != null &&
      ...
      print('=========>    ${_userInfoModel.walletBalance}');
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  void initAddressTypeList(BuildContext context) async {
    if (_addressTypeList.length == 0) {
      ...
      if (apiResponse.response != null &&
        ...
        apiResponse.response.data.map((e) => _addressType = e).toList();

        log('address ----------> ' + _addressType);
      } else {
        ApiChecker.checkApi(context, apiResponse);
      }
      notifyListeners();
  ...
  // save office and home address and location
  void saveHomeAddress(String address) {
    profileRepo.saveHomeAddress(address).then((_) {
      notifyListeners();
    });
  }
