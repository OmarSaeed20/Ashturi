      ...
      bottomNavigationBar: InkWell(
        onTap: () async {
          if(Provider.of<OrderProvider>(context, listen: false).addressIndex == null) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(getTranslated('select_a_shipping_address', context) , style: TextStyle(
                                                              fontFamily:
                                                                  "Ubuntu"),), backgroundColor: Colors.red));
          }else if(Provider.of<OrderProvider>(context, listen: false).billingAddressIndex == null && _billingAddress){
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(getTranslated('select_a_billing_address', context) , style: TextStyle(
                                                              fontFamily:
                                                                  "Ubuntu"),), backgroundColor: Colors.red));
          }
                            ...
                            child: ClipRRect(
                              ...
                              child: Image.network(
                                '${Provider.of<SplashProvider>(context,listen: false).baseUrls.productThumbnailUrl}'
                                    '/${Provider.of<CartProvider>(context,listen: false).cartList[index].thumbnail}', fit: BoxFit.cover, width: 50, height: 50,
                              ),
                            ),
                          ),
